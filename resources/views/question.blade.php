@extends('layouts.app')

@section('content')
    <div class="container">
        <div class="row ">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header" style="background-color: #1d2124; color: #f8fafc; font-size: large">Question</div>

                    <div class="card-body">

                        {{$question->body}}
                    </div>
                    @if (Auth::user())
                        @if(Auth::user() == $question->user)
                            <div class="card-footer">
                                <a class="btn btn-primary float-right" style="background-color: mediumblue"
                                   href="{{ route('question.edit',['id'=> $question->id])}}">
                                    Edit Question
                                </a>

                                {{ Form::open(['method'  => 'DELETE', 'route' => ['question.destroy', $question->id]])}}
                                <button class="btn btn-danger float-right mr-2" value="submit" type="submit" id="submit">Delete
                                </button>
                                {!! Form::close() !!}
                            </div>
                        @endif
                    @endif

                </div>
            </div>







            <div class="col-md-4">
                <div class="card">
                    <div class="card-header"><a class="btn btn-primary float-left" style="background-color: mediumblue"
                                                href="{{ route('answers.create', ['question_id'=> $question->id])}}">
                            Answer Question
                        </a></div>

                    <div class="card-body">
                        @forelse($question->answers as $answer)
                            <div class="card">
                                <div class="card-header" style="background-color: #1d2124; color: #e2e6ea">
                                    <small style="color: #fefefe">
                                        @if (\App\Profile::find ($answer->user_id))
                                            Created By: {{ \App\Profile::find ($answer->user_id)->fname }}
                                        @else
                                            Created By: {{ \App\User::find($answer->user_id)->email }}
                                        @endif
                                    </small>
                                </div>
                                <div class="card-body">{{$answer->body}}</div>
                                <div class="card-footer">
                                    @if (Auth::user())
                                        {{--                                                    @if(Auth::user() != $question->user)--}}
                                        <div class="votes-area" data-answerid="{{ $answer->id }}"
                                             data-userid="{{ Auth::user()->id }}">


                                            <img id="upvote{{$answer->id}}" class="voter" data-votetype="up"
                                                 src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRw9lFyg_qYEks4q-6c_Yr4oBG1lPtFtodNGCSeOgf6nJdjL50KSQ"
                                                 width="20" height="20" >
                                            <span>{{ $answer->votes_up }}</span>

                                            {{--                                                            <a class="btn btn-primary float-right" style="background-color: mediumblue" href="{{ route('question.vote', ['id' => $question->id]) }}">--}}
                                            {{--                                                                {{ $question->votes_up }}--}}
                                            {{--                                                            </a>--}}
{{--                                            <span id="resultVote">{{ $answer->answer_result }}</span><br>--}}
                                            <img id="downvote{{$answer->id}}" class="voter" data-votetype="down"
                                                 src="data:image/jpeg;base64,"
                                                 width="20" height="20">
                                            <span>{{ $answer->votes_down }}</span>
                                            <a class="btn btn-primary float-right" style="background-color: mediumblue"
                                               href="{{ route('answers.show', ['question_id'=> $question->id,'answer_id' => $answer->id]) }}">
                                                {{--                                            href="#">--}}
                                                View
                                            </a>
                                            {{--                                                            <span class="voter" data-votetype="down">{{ $question->votes_down }}</span>--}}
                                        </div>
                                        {{--                                                    @endif--}}
                                    @else
                                        <div class="votes-area">
                                            <span onclick="alert('Only registered users can vote');">{{ $answer->votes_up }}</span>
                                            <span onclick="alert('Only registered users can vote');">{{ $answer->answer_result }}</span>
                                            <span onclick="alert('Only registered users can vote');">{{ $answer->votes_down }}</span>
                                        </div>
                                    @endif



                                </div>
                            </div>
                        @empty
                            <div class="card">

                                <div class="card-body"> No Answers</div>
                            </div>



                            @endforelse




                    </div>
                </div>
            </div>
        </div>
    </div>

            <script type="text/javascript">
                window.onload = function () {
                    var token = "{{ Session::token() }}";
                    var url = "{{ route('answer.vote') }}";
                    var voteSpans = document.getElementsByClassName("voter");
                    //var voteSpans = document.querySelectorAll("voter");
                    // console.log(voteSpans.length);
                    for (var i = 0; i < voteSpans.length; i++) {
                        //   console.log("on click");
                        voteSpans[i].addEventListener('click', vote, true);
                        //voteSpans[i].addEventListener('click', vote, false);
                        //voteSpans[i].onclick =  vote;
                        // console.log(voteSpans[i]);
                    }

                    function vote() {

                        var voteType = this.dataset.votetype;
                        var parentDiv = this.parentNode;
                        console.log(parentDiv.dataset);
                       // var postId = parentDiv.dataset.postid;
                        var userId = parentDiv.dataset.userid;
                        var answerId = parentDiv.dataset.answerid;
                        var spanClicked = this;
                        var xhr = new XMLHttpRequest();
                        xhr.onreadystatechange = function () {
                            if (xhr.readyState == 4 && xhr.status == 200) {
                                var obj = JSON.parse(xhr.responseText);
                                console.log(obj);
                                if ("warning" in obj) {
                                    var pWarning = document.createElement("p");
                                    pWarning.textContent = obj.warning;
                                    spanClicked.parentNode.appendChild(pWarning);
                                    setTimeout(function functionName() {
                                        spanClicked.parentNode.removeChild(pWarning);
                                    }, 3000);
                                } else {
                                    if (spanClicked.dataset.votetype == "up") {
                                        changeImageUp(answerId);
                                        spanClicked.nextElementSibling.textContent = obj.up;
                                       // console.log(spanClicked.nextElementSibling.nextElementSibling);
                                        //spanClicked.nextElementSibling.nextElementSibling.nextElementSibling.textContent = obj.result;
                                        spanClicked.nextElementSibling.nextElementSibling.nextElementSibling.textContent = obj.down;
                                    } else {
                                        changeImageDown(answerId);
                                        spanClicked.nextElementSibling.textContent = obj.down;
                                        //spanClicked.previousElementSibling.previousElementSibling.textContent = obj.result;
                                        spanClicked.previousElementSibling.textContent = obj.up;
                                    }
                                }
                            }
                        }
                        xhr.open('POST', url, true);
                        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;");
                        xhr.send("&userId=" + userId + "&answerId=" + answerId + "&voteType=" + voteType + "&_token=" + token);
                    }
                    function changeImageUp(answerId){
                        document.getElementById("upvote"+answerId).src = "http://i.stack.imgur.com/chwM4.png";
                        document.getElementById("downvote"+answerId).src = "data:image/jpeg;base64,";
                    }
                    function changeImageDown(answerId){
                        document.getElementById("downvote"+answerId).src = "http://www.clker.com/cliparts/H/X/b/b/5/e/thumbs-dwn-icon-red-th.svg.med.png";
                        document.getElementById("upvote"+answerId).src = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRw9lFyg_qYEks4q-6c_Yr4oBG1lPtFtodNGCSeOgf6nJdjL50KSQ";
                    };

                }
            </script>
@endsection